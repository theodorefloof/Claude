...
5. Journey roles are "the advocate (Tori persona)" and "the recipient (Blake persona)" on first use, then "the advocate" and "the recipient". Screens and scripts use the persona names chosen at D1-05 and applied to the brief at D1-06 (placeholders Maya and Sam); script lines say "[advocate persona]" and "[recipient persona]"; never a founder's name.
...
|---|---|---|---|
| T0 Before we talk | `<name>_v1.md` and `<name>_v2.md` on each founder's own device; `01_answers/<name>_v1_receipt.md` and `_v2_receipt.md` in the folder; v2 also printed and sealed | Alone, opened at D1-02; each founder | v1 Sat 19 Sep 22:00 UK; v2 Sun 4 Oct 21:00 UK |
| T1 Company thesis | `01_answers/thesis_one_pager.md` | D1-02, C1, D1-09; Tory | v1 Sat 10 Oct 17:50; final Sun 11 Oct 13:30 |
| T2 Product definition | `02_product/product_definition.md` | D1-05, D2-05; Blake | v1 Sat 10 Oct 11:50; final Sun 11 Oct 13:30 |
| T3 Differentiation thesis | `03_commercial/differentiation_thesis.md` | D1-04, C2; Tory | v0 Sat 10 Oct 11:10; v1 17:15; final Sun 11 Oct 13:30 |
| T4 Sales strategy | `03_commercial/sales_strategy.md` | C1, C2, D1-09; Tory | v1 Sat 10 Oct 17:50; final Sun 11 Oct 11:50 |
| T5 Narrative and deck | `03_commercial/fundraising_narrative.md`; `deck_draft_v1.md` | C2, D2-02, C3; Tory | Narrative Sat 10 Oct 17:15; v2 Sun 11 Oct 09:30; slides 10:25; final 13:30 |
| T6 Product priorities | `02_product/product_priorities_v1.md` | Prep pack, D2-05; Blake | Draft Sat 26 Sep 22:00 UK; final Sun 11 Oct 11:50 |
| T7 Demo script and limitations | `04_prototype/demo_script_v2.md`; `known_limitations_v1.md` | AR1, C3, C4; Tory (script), both (limitations) | v1 Sat 10 Oct 17:15; v2 Sun 11 Oct 11:15 |
| T8 Execution plan | `06_decision_log/execution_plan_v1.md` | D2-05, D2-07; Tory | v1 Sun 11 Oct 11:50; final 14:30 inside the single source document |
| T9 Scope lock form | `04_prototype/scope_lock.md` | D1-06; Blake presents, Tory signs | Signed Sat 10 Oct 12:10 |
| T10 Log entry and trigger | `06_decision_log/decision_log.md`; `parking_lot.md`; kept as sheets of `06_decision_log/live_tables.xlsx` during the workshop, exported to `.md` at D2-07 | Every session; the recorder writes, Tory owns | Closed Sun 11 Oct 14:30 |
| T11 Conversation note | `03_commercial/call_notes_N.md`; `07_evidence/intercept_notes.md`; `evidence_prototype_tests.md` | After each call or test; Tory (brand), Blake (intercepts, cold tests) | Same evening 22:00 UK; sheet v1 Sat 3 Oct 22:00 UK, v2 Thu 8 Oct 22:00 UK; cold tests Wed 14 Oct 22:00 UK |
| T12 Final review checklist | `00_programme/output_status.md`; `01_answers/five_answers.md` | D2-06; Tory leads, Blake records | Sun 11 Oct 13:30; hard stop 14:00; sent 14:30 |

### 6.1 T0 Before we talk (individual, sealed)

T0 makes each founder write alone so D1-02 compares positions rather than composing them; the file stays on its author's device and only a one-line receipt reaches the shared folder.
...
v2 (Sun 4 Oct 21:00 UK) keeps the seven headings and adds one line under each, "changed" or "unchanged, because", after reading the other founder's prep pack; still sealed. The receipt (`01_answers/blake_v1_receipt.md`) reads "written Sat 19 Sep, one page, seven headings, sealed". Opened at D1-02 step 2 (Sat 10 Oct 09:16, envelopes opened together) for T0.1 to T0.7 (d) and at D1-03 (10:00) for T0.7 (e); each line marked A (agree), D (differ) or U (unsure); nothing edited after opening.
...
T1 answers Q1 in a form both founders would say to a stranger, with every claim labelled; Tory scaffolds it at C1 from the answer blocks and closes it at D1-09.
...
| T1.4 Founding insight | Two lines in Tory's words, including what "a million times better" originally meant and for whom | Commercial prep pack; T3.3; Tory's originator view verbatim where the founders differ |
...
| T1.9 What we know | FACT rows only, each with a file and a date (for example "three brand calls held, dates, `03_commercial/brand_notes_v2.md`"; "no community leader contacted") | reuse from T11 summaries and `07_evidence/`; C1 scaffold; updated at D1-09 step 4 (thesis v1 at 17:50) |
...
|---|---|---|
| T2.1 Core experience | One paragraph: the advocate (Tori persona) keeps a Locker of real products with personal reasons and shows a Sway Pass or sends a Sway Link; the recipient (Blake persona) lands on a recommendation page with one dominant action; the advocate sees observable outcomes as Trust | D1-05 step 5; FOUNDER DECISION |
...
| T2.4 Attribution event table | Rows: scan; view; save; click; purchase where visible; repeat; downstream recommendation. Event plus seven data columns: What is known; What is inferred; How it can be faked and the counts-once rule; Moves Trust?; Moves Sway Points?; Shown to the advocate as; Claimable to a brand as (auditable by a finance team without an integration). Pre-drafted in full by Blake in the product prep pack (Sat 26 Sep); Tory pre-fills the last column and marks the rest A, D or U on Sun 4 Oct; D1-05 step 3 edits and agrees | D1-05 step 3, deferred rows at D2-05; FOUNDER DECISION per row |
...
| T2.7 Persona names | Advocate name; recipient name; shown beside role labels on screen | D1-05 step 4; reused in T7.1 and T9.1 |
...
|---|---|---|---|
| A | T3.1 What Duel demonstrably offers | Fact; URL; grade | Public pages only; Tory writes, Blake adds nothing and the abstention is logged; a row without Tory's dated screenshot is struck |
...
- T3.8 "Who owns the advocate" paragraph: identity; consent; export; what a brand keeps when a contract ends. Blake writes the product half, Tory the commercial half.
...
T4 is Tory's nine-element sales strategy, drafted in C1 and C2 while Blake builds and decided at D1-09 against what the prototype shows at CP4, with every number labelled.
...
| T4.2 ICP | Five lines: brand type; size signal; programme run today; the person who feels the pain; the pain in their words; the launch city or "not yet chosen" (D1-09) | C1; verbatim from T11 where a call was held |
...
| T4.11 Q3 answer block | Under 150 words: user, buyer, budget holder, trigger, value demonstration, pricing hypotheses, first customers; Provisional with the trigger "three brand conversations held by Fri 23 Oct 22:00 UK, owner Tory" if fewer than two calls were held | D1-09 step 4; labelled at D2-06 |

The nine-prompt check in 4B.10 is Tory's self-check on Thu 15 Oct 22:00 UK (or during D2-04X in the extension variant); it may add OPEN QUESTION rows and never changes a decision.
...
| T7.3 Six beats | Beat; who acts (founder Blake as the advocate; founder Tory as the recipient; script lines say "[advocate persona]" and "[recipient persona]"); line said; acceptance test number; the T3.6 claim it shows; limitation said aloud | T9.4 (tests); T3.6 (claims) |
...
T9 is the one-page form Tory signs at 12:10 on Sat 10 Oct so the build starts from a signature, not a debate; Blake's Sun 4 Oct scope proposal is pasted in and only the D1-05 decisions change it.

This form, printed on Thu 8 Oct, is the sheet Tory signs at 12:10; 4A.8 and 5.2 point here and hold no second version of it.

| Field | Content | Source |
|---|---|---|
| T9.1 Journey and personas | One line: hero mechanic as decided; personal Pass or product-specific QR first; persona names | T10 entry D1-02-a; T2.7 |
...
| T9.7 Tory's three challenges | Challenge (from `06_decision_log/divergence_list_v1.md`, Tue 6 Oct 22:00 UK); Blake's one-line answer; accepted or logged | D1-06 step 1 |
| T9.8 Lock | "Build brief v2 pasted into the tool's project instructions at [time]"; "No new features after 12:10; new ideas go to the parking lot"; Tory's signature and time; Blake's initials; log entry "scope locked 12:10" | D1-06 steps 4 and 5 |
...
8. T10.8 Dissent verbatim: required for Provisional; Tory's originator view recorded verbatim on Q1 and Q5 whenever the founders differ.
...
During the workshop the log, the parking lot, the checkpoint log and the fix lists live as one sheet each in `06_decision_log/live_tables.xlsx` (programme choice); the recorder types into the sheet and Tory exports each to its `.md` file at D2-07.

Worked example from the rules card (replace with the real entry): "D1-02-a | 09:30 | D1-02 | hero mechanic | Hero mechanic = Sway Pass | Provisional | T0.7 (a) both founders; alternatives_landscape.md section 4, [SNIPPET] | Tory: 'the card in the chat will be used more' | reopen if the Pass is offered in fewer than one in five recommendation moments in the six cold tests and the first cohort's first two weeks, or if median scan-to-page exceeds 15 seconds; check Sun 18 Oct 21:00 UK and Fri 20 Nov 22:00 UK; owner Blake | the demo leads with the Link; T2.2, T7.1 and T9.1 change | six cold tests | Blake | Sun 18 Oct 21:00 UK."

Other trigger forms to use: "reopen if fewer than 3 of 10 cold-test recipients tap the dominant action first, by Fri 20 Nov 22:00 UK, owner Blake"; "reopen if the first brand marketer to hear the offer names a different budget line, by Fri 23 Oct 22:00 UK, owner Tory".

Parking lot (`06_decision_log/parking_lot.md`), six columns: Idea; Raised by; Time; Session; Triaged at (D1-10, D2-01 or D2-06); Outcome (execution plan row, dropped, or logged as Open question). Review-trigger tracker (`06_decision_log/review_triggers.md`; Tory; Mon 12 Oct 22:00 UK): one row per Provisional decision and Open question: ID; trigger observation; check date; owner; status (holds; triggered; awaiting a dated event); reuse from T10.9.
...
T11 is one note per conversation in the same fields every time, so verbatim answers can be counted, dated and attributed before anything is called a Validated finding.
...
Summary fields for brand calls, copied into `03_commercial/brand_notes_v1.md` (Sat 3 Oct 22:00 UK) and `brand_notes_v2.md` (Thu 8 Oct 22:00 UK):
...
Example header (example, replace): "T11.1 brand marketer | Tue 22 Sep 19:30 UK, 30 min | Head of Community, DTC supplement brand, second-degree LinkedIn, Tory attended | recorded yes; anonymised quotes yes; follow-up yes."

Counting rule: a Validated finding needs a count, a date and the reporting founder's name from five or more observations; fewer than two brand calls is recorded as "no brand evidence" at D1-09.
...
Part 1, the consistency checklist (17 checks, identical to 4B.11); a fail is fixed inside D2-CT or labelled Draft at 14:00:
